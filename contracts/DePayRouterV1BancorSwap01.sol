...
contract DePayRouterV1BancorSwap01 {
  ...
  // Swaps tokens according to provided `path` using
  // the amount at index 0 (`amounts[0]`) as input amount,
  // the amount at index 1 (`amounts[1]`) as expecting output amount
  // the amount at index 2 (`amounts[2]`) as packed fee and sqrtPriceLimitX96
  //
  function execute(
